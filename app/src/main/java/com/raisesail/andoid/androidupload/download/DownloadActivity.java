...
import com.raisesail.andoid.androidupload.bean.ApkModel;
...
public class DownloadActivity extends AppCompatActivity {
    private TextView mTargetFolderTextView;
    ...
    private void initData() {
        OkDownload.getInstance().setFolder(Environment.getExternalStorageDirectory().getAbsolutePath() + "/ABK/");
        OkDownload.getInstance().getThreadPool().setCorePoolSize(3);

        mTargetFolderTextView.setText(String.format("下载路径: %s", OkDownload.getInstance().getFolder()));
...
//            startActivity(intent);
    ...
    private void initView() {
        mTargetFolderTextView = findViewById(R.id.targetFolder);
